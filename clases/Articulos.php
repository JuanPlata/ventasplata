...
<?php 
	class articulos{
		...
		public function actualizacantidadArticulo($idart){
			$c=new conectar();
			$conexion=$c->conexion();
			$cant =self::idproducto($idart);
			$sql="UPDATE articulos set cantidad='$cant'
		...
		public function idproducto($idart){
			$c=new conectar();
			$conexion=$c->conexion();

			$sql="SELECT COUNT(id_producto) 
					from ventas 
					where id_producto='$idart'"; 
			$result=mysqli_query($conexion,$sql);
